...
namespace gmf
{
    ...
    Message::PARSE_RESULT_E Gmf::ParseBinary(std::shared_ptr<IOInterface> io) {
        ...
        BINARY_HEADER_T *pHead = (BINARY_HEADER_T *)header.data();

        /** Determine whether it is a Gmf message */
        if (this->GetMessageID() != pHead->MsgID) {
            return Message::PARSE_RESULT_UNFINDHEADER;
        }

        /** Peek Complete message */
        uint32_t expect_size = sizeof(BINARY_HEADER_T) + pHead->BodyLen + 4; /**< CRC 4 byte */
        ...
        /** Calculate CRC */
        if (CalculateCRC32(message, 0, expect_size - 4) != *(uint32_t *)(message.data() + expect_size - 4)) {
            MessageClassGmf::msg_basic_info.MsgID = pHead->MsgID;
    ...
    bool Gmf::IsSlaveAntMessage(const std::vector<uint8_t> &msg) {
        return (0 != (((BINARY_HEADER_T *)msg.data())->MsgAttribute & 0x01));
    }
} /* namespace gmf */
} /* namespace messsage */
} /* namespace huace_m7xx_ros_driver */
