...
namespace huace_m7xx_ros_driver {
    ...
    static std::string Uint8ToHexString(uint8_t num) {
        return {
            hex_table[(num >> 4) & 0x0F], // high four bit
            hex_table[num & 0x0F]         // low four bit
        };
    }

    static std::vector<uint8_t> Uint8ToHexVector(uint8_t num) {
        return {
            (uint8_t)hex_table[(num >> 4) & 0x0F], // high four bit
            (uint8_t)hex_table[num & 0x0F]         // low four bit
        };
    }
    ...
    uint8_t CalculateXOR(const std::shared_ptr<std::string> str, uint32_t offset, uint32_t len) {
        ...
        return CalculateXOR(*str, offset, len);
    }
    ...
    uint8_t CalculateXOR(const std::shared_ptr<std::vector<uint8_t>> vec, uint32_t offset, uint32_t len) {
        ...
        return CalculateXOR(*vec, offset, len);
    }
    ...
    const static uint32_t CRC32Table[256] =
    {
    ...
    static uint32_t CalculateCRC32(uint8_t *buffer, uint32_t length)
    {
        uint32_t crc = 0;
        for(uint32_t ichar = 0; ichar < length; ichar++)
        {
            crc = CRC32Table[(crc ^  buffer[ichar]) & 0xff] ^ (crc >> 8);
        }
        return crc;
    }

    uint32_t CalculateCRC32(const std::vector<uint8_t> &vec, uint32_t offset, uint32_t len) {
        return CalculateCRC32((uint8_t *)vec.data() + offset, len);
    }
    ...
    uint32_t CalculateCRC32(uint8_t *buffer, uint32_t offset, uint32_t len) {
        return CalculateCRC32(buffer + offset, len);
    }
